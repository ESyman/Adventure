...
function dungeonRoom(){
  roomLocation = 1;
if (dungeonDoor == false && gotPoster == false && gotKey == false) story("You find yourself stuck in a dungeon room. A SKELETON sits nearby you. A POSTER is on the wall. There’s a KEY nearby.");
else (dungeonDoor == false && gotPoster == false && gotKey == true) story("You find yourself stuck in a dungeon room. A SKELETON sits nearby you. A POSTER is on the wall.");
else (dungeonDoor == false && gotPoster == true && gotKey == false) story("You find yourself stuck in a dungeon room. A SKELETON sits nearby you. Torn bits of a POSTER remain on the wall. There's a KEY nearby.");
else (dungeonDoor == false && gotPoster == true && gotKey == true) story("You find yourself stuck in a dungeon room. A SKELETON sits nearby you. Torn bits of a POSTER remain on the wall.");
else (dungeonDoor == true && gotPoster == false) story("You find yourself stuck in a room of somebody's basement, and not a dungeon. A SKELETON sits nearby you. A POSTER is on the wall.");
else (dungeonDoor == true && gotPoster == true) story("You find yourself stuck in a room of somebody's basement, and not a dungeon. A SKELETON sits nearby you. Torn bits of a POSTER remain on the wall.");
choices = ["LOOK SKELETON", "LOOK POSTER", "LOOK DOOR"];
answer = setOptions(choices);
}
function lookSkeleton(){ // 1.2 S2 - Look at skeleton
  story("It’s a very cheap looking skeleton, like somebody bought it from a thrift shop, minus the skull which actually looks somewhat convincing from a distance. All of its body, or rather bones, are stuck on the ground.");
  choices = ["GET SKELETON", "TALK SKELETON", "GO BACK"];
  answer = setOptions(choices);
}
function getSkeleton(){ // 1.2 s2 C1 - get skeleton
  if (gotClip == false) {
story("You try to grab the skeleton but you find out that it’s stuck to the ground. On the plus side, you find a clip in it and you pick it up. Don't ask me where you found it, though.");
    gotClip = true;
  ...
  else if (gotGloves == true && gotSkeleton < 3) {
	  story("Even with the gloves on, it's a futile attempt to try to get the skeleton. Maybe if you pull a little harder... actually, don't try.");
	  gotSkeleton++;
  } else if (gotGloves == true && gotSkeleton == 3) {
	  story("You successfully grab the skull from the skeleton. You look into it, wondering if it's a prop or not, unlike the rest of the body... and then a skeleton popped out, instantly killing you from the sheer horror (or lack thereof). Maybe you shouldn't have read so many bad creepypastas. \nGAME OVER. Play again?")
	  choices = ["Y", "N"];
    answer = setOptions(choices);
  }
} 
function talkSkeleton(){ //1.2 S2 C2 - Talk to skeleton
  if (skeletonTalk < 3){
...
function lookPoster(){ //1.2 S3 - look at poster
...
function getPoster(){ // 1.2 S3 C1 - get poster
  if (gotPoster == false) {
...
function lookKey(){ //1.2 S4 - look at key
...
function getKey(){ //1.2 S4 C1 - get key
  if (gotKey == false) {
...
function lookDungeonDoor(){ //1.2 S5 - Look at dungeon door
...
function openDungeonDoor(){ // 1.2 S5 C1 - Open dungeon door
...
function mainRoom(){
 ...
 if (seenMain == false) {
	 story("You're now in what appears to be somebody's basement. Perhaps that last room wasn't a dungeon but a really poorly kept room. There's a door to the SOUTH leading back into said room, as well as another door to the NORTH. There are some GLOVES nearby.");
 seenMain == true}
 else if (gotGloves == false) story("You're now in what appears to be somebody's basement. There's a door to the SOUTH leading back into said room, as well as another door to the NORTH. There are some GLOVES nearby.");
 else story("You're now in what appears to be somebody's basement. There's a door to the SOUTH leading back into said room, as well as another door to the NORTH. There are definitely not some GLOVES sitting in the room.");
...
function lookBag(){ // 1.4 S3 - look at bag
  if (gotGiantKey == true){
    story("You get distracted by the bag with assorted goodies inside, and... oh no! A hand reached out of it and took your giant key, taking it inside the bag. You jump inside it in an attempt to get it back, but now you're trapped in it. Great going. \nGAME OVER. Play again?");
    choices = ["Y", "N"];
  } else {
...
function getBag(){ //1.4 S3 C1 - get bag
  story("You try to grab the bag, but before you do, a hand reaches out of it, grabbing you in the process and stuffing you into it. Great job; you're now stuck in a magical bag with who knows what's inside it. GAME OVER. Play again?");
choices = ["Y", "N"];
answer = setOptions(choices);
}
function getGoodies(){  //1.4 S3 C2 - get goodies
  story("You try to grab some of the goodies within the bag, but before you do, a hand reaches out of it, grabbing you in the process and stuffing you into it. Sure, you got the goodies with you, but you're stuck in the bag. GAME OVER. Play again?");
choices = ["Y", "N"];
answer = setOptions(choices);
}
...
function openEastwardsDoor(){ //1.4 S2 C1 - open eastwards door
  if (gotPoster == false) {
    ...
    story("You decide to look at the poster, and realize that those numbers on it could be a code! You enter it in and the door unlocks itself. Congrats.");
    eastDoor = true;
    choices = ["CONTINUE"];
    answer = setOptions(choices);
...
function eastRoom(){
  roomLocation = 4;
  if (gotGiantKey == false) story("You find yourself in an empty room, minus the GIANT KEY in the middle of it on a pedestal. The door leads back to the room you came from.");
  else story ("You find yourself in an empty room, minus the pedestal that once hosted a giant key. The door leads back to the room you came from.");
